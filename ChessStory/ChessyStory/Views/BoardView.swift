...
class BoardView: UIView {
    ...
    //TODO because UIView doesn't work right
   ...
   //}
    ...
    //Import userX and userY to location
    ...
    func generateMoveGraphs(){
        ...
        func findPath(movesGraph: [Position: Node], cutoff: Int = 3) -> [Node]? {
            ...
            if var path = recrFindPath(v: root!, depth: 0, cutoff: 3, dest: dest!)//returns path array of nodes
            {
                path.reverse()
                return path
            }
        ...
        if let path = findPath(movesGraph: g){
          ...
          print("Starting position and ending position are the same.")
          resultString += "Starting position and ending position are the same.\n"
        } else {
            print("No path found.")
